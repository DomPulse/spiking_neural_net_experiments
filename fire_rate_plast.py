...
def update_net(local_tslfs, local_mem_volt, local_neur_params, local_syn_weights, local_fired, local_g_E, local_g_I):
	...
	if training:
		...
		local_neur_params[:,3] += del_t * (-1*local_neur_params[:,3] + fired[num_input:]*50000)/(7000*Tau)
...
for e in range(1, epoch):
	...
	for b in range(batch_size):
		...
		for s in range(sim_steps):
			fired[:num_input] = convert_to_binary_1D_normalized(0.1*train_data[data_idx][0]/np.mean(train_data[data_idx][0]), input_prob) #this auto normalizes input strength (hopefully)
		...
		is_right = (train_labels[data_idx] == np.argmax(sum_output_fires)) and (1 == np.count_nonzero(sum_output_fires == np.max(sum_output_fires)))
		num_right += is_right
		...
		if e > 11:
			...
			sum_fire_each_neuron /= max_expected_fire
	...
	print(e, num_right/batch_size_train, mean_fires_out, np.mean(neur_params[:, 3]), np.mean(real_jFs[num_input:num_input+num_hidden_exc,:]), np.mean(synapses))
