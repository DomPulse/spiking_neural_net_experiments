...
def load_and_display_image(npy_file, subfolder_index, image_index):
	# Load the saved NumPy file
	image_data = np.load(npy_file, allow_pickle=True).item()
	
	subfolder_keys = sorted(image_data.keys())
	if subfolder_index < 0 or subfolder_index >= len(subfolder_keys):
		print("Invalid subfolder index")
		return
	
	subfolder = subfolder_keys[subfolder_index]
	images = image_data[subfolder]
...
def cherry_pie(c, c_avg, c0):
	...
	for pair in idx_pairs:
		...
		if exin_array[pre_syn_idx] == 1:
			...
			dum = np.mean(c[post_syn_idx + num_input]**2)/(norm_inhib)
			dums.append(dum)
...
for pre_syn_idx in range(num_all):
	...
	for post_syn_idx in range(num_neurons):
		...
		elif adjusted_pre_syn_idx < num_hid and post_syn_idx < num_hid: #in to hid
			synapses[post_syn_idx, pre_syn_idx] = np.max([np.random.normal(weight_tune, weight_std), 0])
			defaults[post_syn_idx, pre_syn_idx] = weight_tune
			idx_pairs.append([pre_syn_idx, post_syn_idx])
...
train_fire_mask = np.ones((10, num_all, sim_steps))
train_fire_mask[:, num_input + num_hid:, ::2] = np.zeros((10, num_out, sim_steps//2))
for n in range(num_out):
	train_fire_mask[n%10, num_input + num_hid + n] = np.ones(sim_steps)
...
for e in range(epoch):
	...
	for b in range(this_batch_please):
		train_class = np.random.randint(0, 10)
		train_idx = np.random.randint(0, 101)
		data = load_and_display_image(img_path, train_class, train_idx)/255
		data /= np.mean(data)
		data *= 0.33
		#print(np.mean(data))
		#train_class = 0
		...
		for s in range(sim_steps):
			fired[:num_input] = convert_to_binary_1D_normalized(data, test_stim_stren)
