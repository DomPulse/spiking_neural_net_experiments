...
epoch = 20
stren_mults = 0.5*np.exp(-1*np.linspace(0, 2, epoch))
print(stren_mults)
batch_size_train = 25
learning_rate = 2

sim_length = 500 #number of miliseconds in real time
...
def cherry_pie(c, c_avg):
	...
	for pair in idx_pairs:
		...
		d = c[pre_syn_idx]
		del_synapses[post_syn_idx][pre_syn_idx] += direction*np.mean(phi(c[post_syn_idx + num_input], c_avg[post_syn_idx + num_input], 0.8)*d)
...
mean_region_ot = np.zeros((epoch, 2, sim_steps-look_back+1))
for e in range(epoch):
	...
	for b in range(batch_size_train):
		...
		for s in range(sim_steps):
			fired[:num_input] = convert_to_binary_1D_normalized(woah_dude[train_class], stren_mults[e])
		...
		mean_region_ot[e, 0, :] += np.sum(smoothed_fires[:num_input], axis = 0)/(num_input*batch_size_train)
		mean_region_ot[e, 1, :] += np.sum(smoothed_fires[num_input:], axis = 0)/(num_out*batch_size_train)
	
for e in range(epoch):
	plt.plot(mean_region_ot[e, 1], label = str(stren_mults[e]))
plt.legend()
plt.show()
